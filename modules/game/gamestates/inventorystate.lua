local InventoryActionState = require "modules.game.gamestates.inventoryactionstate"
...
function InventoryState:update(dt)
   ...
   for i, letter in ipairs(self.letters) do
      if spectrum.Input.key[letter].pressed then
         self.manager:push(
            InventoryActionState(self.display, self.decision, self.level, self.items[i])
         )
      end
   end
